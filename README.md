...
4. **Configure environment variables**
   Create `.env` files in both root and `agents/` directories with:
   ```
   # For frontend (in root)
   VITE_APP_API_KEY=your-amplify-api-key
...
2. **Start the Amplify sandbox (backend)**
   ```bash
   npm run amplify:dev
   ```
...
```bash
npm run build
# Deploy to Amplify Hosting, Vercel, or Netlify
```
